...
const HSCodeFilter = ({
  ...
  const testingFun = (arr) => {
    ...
    arr.forEach((item) => {
      ...
      result.topYearsByValue[year] =
        (result.topYearsByValue[year] || 0) + Number.parseFloat(item.quantity * item.unitPrice)
      ...
      if (info === "import") {
        ...
        result.topBuyerByValue[item.indianCompany] =
          ...
          Number.parseFloat(item.quantity * item.unitPrice)
      ...
      result.totalValue += Number.parseFloat(item.quantity * item.unitPrice)
    })
  ...
  const handleHsGroupSelection = (codes, checked) => {
    // Create a copy of the current filter data for H S Code
    const currentHSCodeFilter = { ...(filterData["H S Code"] || {}) }

    // Update the filter state for all codes in the group
    codes.forEach((code) => {
      currentHSCodeFilter[code] = checked
    })

    // Update the filter data state
    setFilterData((prevFilterData) => ({
      ...prevFilterData,
      ["H S Code"]: currentHSCodeFilter,
    }))

    // Update the current key and previous filter
    setCurrentKey("H S Code")
    setPreviousFilter(currentHSCodeFilter)

    // Filter the data based on the updated selection
    let newFilteredData
    const formattedKey = "HS_Code" // Direct mapping for HS_Code

    if (checked) {
      // When selecting, add all items that match any of the codes
      const additionalItems = allData.filter(
        (item) =>
          codes.includes(String(item[formattedKey])) &&
          !leftFilterData.some((existing) => existing[formattedKey] === item[formattedKey]),
      )
      newFilteredData = [...leftFilterData, ...additionalItems]
    } else {
      // When deselecting, remove all items that match any of the codes
      newFilteredData = leftFilterData.filter((item) => !codes.includes(String(item[formattedKey])))
    }

    // Update the filtered data
    setLeftFilterData(newFilteredData)

    // Update the "Select All" state for H S Code
    const allHSCodes = Object.keys(currentHSCodeFilter)
    const allChecked = allHSCodes.length > 0 && allHSCodes.every((code) => currentHSCodeFilter[code])
    ...
    // Update the graphs with the new filtered data
    return testingFun(newFilteredData)
  }

  const getSingleFilter = (key, value, checked) => {
    const formattedKey = Object.keys(mapping).find((mappingKey) => mapping[mappingKey] === key)

    // Create a copy of the current filter state for this key
    const currentFilterState = { ...(filterData[key] || {}) }
    currentFilterState[value] = checked
    ...
    // Update current key and previous filter
    if (currentKey === key) {
    ...
    if (checked) {
      ...
      newFilteredData = [...leftFilterData, ...filterFromAllData]
    } else {
      newFilteredData = leftFilterData.filter((item) => String(item[formattedKey]) !== String(value))
    }

    // Update the "Select All" state based on whether all items are checked
    const allValuesForKey = Object.keys(currentFilterState)
    const allChecked = allValuesForKey.length > 0 && allValuesForKey.every((val) => currentFilterState[val])
        ...
        {Object.keys(filterData).map((key, index) => (
          <div key={key} className="relative">
            <Popover open={key === activeIndex} onOpenChange={(open) => setActiveIndex(open ? key : null)}>
              ...
              <PopoverContent
                className="w-72 bg-white p-0 ml-[-5px]"
                side="right"
                align="center"
              >
                  ...
                  {key === "H S Code" ? (
                    ...
                    <div className="flex flex-col gap-1 py-1">
                      {Object.keys(filterData[key])
                        ...
                        .sort((a, b) => a.localeCompare(b)) // Alphabetical sort added here
                        .map((value, i) => (
